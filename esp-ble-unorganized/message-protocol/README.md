...
기준: 2026-08-28의 STM32 통합 소스와 ESP32 **Layer 8** 소스. 보드 검증 결과는 별도의 [검증 기록](../verified/03-button-to-mesh-test.md)을 따른다.
...
원본 정의: [STM32 message_type.h](../stm32-project/common/protocol/message_type.h). Layer 8은 [로컬 복사본](../layers/layer-8/common/message_type.h)을 사용한다.
...
현재 D10/PB6 테스트 버튼은 정지 요청을 만든다. [button.c](../stm32-project/integration/stm32/MyApp/hw/button.c)는 LOW를 눌림으로 읽고, 30ms 디바운싱 후 **눌림 전이**에 `MSG_STOP_REQUEST`를 반환한다. 버튼을 계속 누르고 있는 동안 매 반복마다 보내는 구조는 아니다.
...
실제 [uart_service.c](../stm32-project/integration/stm32/MyApp/service/uart_service.c)의 핵심:
...
[main.c](../stm32-project/integration/stm32/Core/Src/main.c)의 `app_init(&hspi2, &huart1, &hi2c1)`가 메시지 통신에 USART1을 연결한다. `1U`는 **1바이트 전송**, timeout 상수는 `10ms`다. enum이나 구조체 전체를 `sizeof(...)`로 보내지 않는다.
...
[bridge_runtime.c](../layers/layer-8/main/bridge_runtime.c)의 `uart_rx_task()`가 UART를 한 바이트씩 읽는다.
...
[event_protocol.c](../layers/layer-8/common/event_protocol.c)의 포장 부분은 다음 두 줄이다:
...
[mesh_node.c](../layers/layer-8/main/mesh_node.c)는 이 2바이트와 Vendor opcode를 Mesh API에 넘긴다.
...
**기존 실물 기록:** [2026-08-28 버튼 시험](../verified/03-button-to-mesh-test.md)에 STM32 → D6 UART → 76 Mesh 수신이 세 번 관찰되어 있다. 이번 문서 작성에서 보드를 다시 시험하지는 않았다. 상대 STM32 수신, 제어된 다중 홉 Relay, reset 직후 안정성 등은 [미검증 항목과 한계](../verified/04-recovery-and-limits.md)를 확인한다.

## 7. 다음에 코드를 볼 순서

| 읽을 파일 | 확인할 내용 |
| --- | --- |
| [message_type.h](../stm32-project/common/protocol/message_type.h) | 메시지 이름과 번호 |
| [button.c](../stm32-project/integration/stm32/MyApp/hw/button.c) | 버튼에서 ID 선택 |
| [app.c](../stm32-project/integration/stm32/MyApp/ap/app.c) · [message_router.c](../stm32-project/integration/stm32/MyApp/service/message_router.c) | 로컬 이벤트를 UART 전송으로 연결 |
| [uart_service.c](../stm32-project/integration/stm32/MyApp/service/uart_service.c) | STM32의 실제 1바이트 송신 |
| [event_protocol.c](../layers/layer-8/common/event_protocol.c) | ID ↔ 2바이트 payload 변환 |
| [event_bridge.c](../layers/layer-8/common/event_bridge.c) | 큐·전송 방향·유효성·만료 |
| [bridge_runtime.c](../layers/layer-8/main/bridge_runtime.c) | UART Task와 송신 worker |
| [mesh_node.c](../layers/layer-8/main/mesh_node.c) | Vendor Model 전송과 수신 callback |
...
[8종 실제 송수신 테스트](../testing/MESSAGE_BROADCAST.md) · [프로젝트 처음으로](../README.md) · [Layer 8 설정·실행 안내](../layers/layer-8/README.md)
