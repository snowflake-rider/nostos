...
usage() {
  printf '%s\n' \
    ...
    '' \
    'Precondition:' \
    '  A separate powered Board A must advertise as ESP32-LAYER-2.' \
    '' \
    'Environment:' \
...
finish() {
  ...
  if [[ "$RESULT" == "PASS" && $exit_code -eq 0 ]]; then
    ...
    printf 'PEER_OVER_AIR_SCAN=PASS\n'
    printf 'SCAN_TARGET_NAME=ESP32-LAYER-2\n'
    printf 'SCAN_TARGET_SERVICE=7A110000-6B0D-4D5A-8F4B-2C9E00000001\n'
  else
    if ((exit_code == 0)); then
      exit_code=1
    fi
...
printf 'ESP32-S3 Layer 3 BLE Active Scanner workflow\n'
printf 'PROJECT_DIR=%s\n' "$PROJECT_DIR"
printf 'RUN_TIMESTAMP=%s\n' "$RUN_TIMESTAMP"
printf 'EXPECTED_PEER=ESP32-LAYER-2\n'
...
command -v idf.py >/dev/null 2>&1 || \
  fail 'idf.py is unavailable after ESP-IDF export.'
command -v esptool.py >/dev/null 2>&1 || \
  fail 'esptool.py is unavailable after ESP-IDF export.'
command -v python >/dev/null 2>&1 || \
  fail 'ESP-IDF Python interpreter is unavailable.'
...
while IFS= read -r serial_port; do
  ...
  find /dev -maxdepth 1 \
    \( -name 'cu.usb*' -o -name 'cu.SLAB*' -o -name 'cu.wchusb*' \) \
...
[[ -c "$PORT" ]] || \
  fail "Selected serial path is not a character device: $PORT"
...
for build_artifact in \
  ...
  build/esp32s3_layer_3.bin; do
  [[ -f "$build_artifact" ]] || \
    fail "Missing build artifact: $build_artifact"
...
print_stage "target_reception_verification"

printf 'Waiting up to %s seconds for %s from powered Board A...\n' \
...
try:
    with serial.Serial(port, baudrate=baud, timeout=0.25) as device:
        ...
        while time.monotonic() < deadline:
            ...
            if marker in line:
                print("TARGET_RUNTIME_MARKER_FOUND=yes", flush=True)
                raise SystemExit(0)
except serial.SerialException as error:
    print(f"SERIAL_ERROR={error}", file=sys.stderr, flush=True)
    raise SystemExit(2)

print("TARGET_RUNTIME_MARKER_FOUND=no", file=sys.stderr, flush=True)
raise SystemExit(1)
PY
then
  fail 'Layer 3 did not receive ESP32-LAYER-2 from Board A.'
fi
