...
usage() {
  printf '%s\n' \
    ...
    '' \
    'This verifies one board local runtime. Use bootload-pair.sh for PACKET_RX_NEW.' \
    '' \
    'Environment:' \
...
finish() {
  ...
  if [[ "$RESULT" == "PASS" && $exit_code -eq 0 ]]; then
    ...
    printf 'LOCAL_PACKET_TX=PASS\n'
    printf 'PAIR_PACKET_RX=NOT_VERIFIED\n'
  else
    if ((exit_code == 0)); then
      exit_code=1
    fi
...
printf 'ESP32-S3 Layer 5 one-board packet-node workflow\n'
    ...
    with serial.Serial(port, baudrate=baud, timeout=0.25) as device:
        ...
        while time.monotonic() < deadline:
            ...
            if marker in line and "gatt=yes" in line and \
                ...
                runtime_ready = True
            if "[LAYER-5] PACKET_TX " in line and \
                    "payload=HELLO" in line and "crc=ok" in line:
                packet_tx = True
            if runtime_ready and packet_tx:
                print("LOCAL_RUNTIME_MARKER_FOUND=yes", flush=True)
                print("LOCAL_PACKET_TX_FOUND=yes", flush=True)
                raise SystemExit(0)
...
print(f"LOCAL_PACKET_TX_FOUND={'yes' if packet_tx else 'no'}",
      file=sys.stderr, flush=True)
raise SystemExit(1)
PY
then
  fail "Local packet-node runtime marker was not received from $PORT."
fi
