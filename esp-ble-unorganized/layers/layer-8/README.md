...
현재 단일 버튼 실물 시험은 **STM32 D10/PB6 버튼 → USART1 D8/PA9 TX → ESP32 GPIO18** 구성이다. USART2는 ST-LINK USB 진단 복사본에만 사용한다. 배선과 자동 관찰 방법은 [빠른 버튼 → Mesh 검증](FAST_CHECK.md), 실제로 확인된 경계는 [STM32 버튼 시험 기록](../../stm32-project/integration/stm32/BUTTON_UART_TEST.md)을 따른다. USB 진단 복사본 수신만으로 D8에서 ESP32까지의 전기적 전달을 입증하지 않는다.
...
4. [common/event_protocol.c](common/event_protocol.c): 1바이트 ID ↔ 2바이트 Mesh payload.
5. [common/event_bridge.c](common/event_bridge.c): 큐·만료·방향·자기 메시지 제외.
...
[Layers 목록](../README.md) · [Layer 로드맵](../../docs/02-learning/LAYER-ROADMAP.md)
