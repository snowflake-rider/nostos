...
| 파일 | 확인한 내용 / 이후 수정할 부분 |
| --- | --- |
| [로컬 VCP 예제 README](/Users/kafka/esp/esp-idf-v5.5.5/examples/peripherals/usb/host/cdc/cdc_acm_vcp/README.md) | USB-UART VCP 장치 연결 방법 |
| [로컬 idf_component.yml](/Users/kafka/esp/esp-idf-v5.5.5/examples/peripherals/usb/host/cdc/cdc_acm_vcp/main/idf_component.yml) | `usb_host_cp210x_vcp`, `usb_host_vcp` 등 의존성 |
| [로컬 cdc_acm_vcp_example_main.cpp](/Users/kafka/esp/esp-idf-v5.5.5/examples/peripherals/usb/host/cdc/cdc_acm_vcp/main/cdc_acm_vcp_example_main.cpp) | Host 설치, VCP 등록·열기, line coding, RX callback |
...
현재 Layer 8은 [sdkconfig.defaults](../layers/layer-8/sdkconfig.defaults)에 `CONFIG_ESP_CONSOLE_USB_SERIAL_JTAG=y`가 있고, [main.c](../layers/layer-8/main/main.c)의 컴파일 조건도 이를 요구한다. 따라서 나중에 USB Host를 통합하려면 **콘솔 설정·컴파일 조건·콘솔 입출력을 함께 검토**해야 한다. COM에 Mac을 꽂았다는 사실만으로 현재 앱의 로그 경로가 자동 변경되지는 않는다.
...
현재 [bridge_runtime.c](../layers/layer-8/main/bridge_runtime.c)는 **UART1 / TX GPIO17 / RX GPIO18 / 115200**을 STM32↔Mesh 통신에 사용한다. [현재 핀 기록](../settings/PIN.md)도 이 경로를 설명한다. 따라서 C1용으로 기존 UART1을 재초기화하거나 17/18에 센서를 끼워 넣지 않는다. **UART2 + 별도 GPIO**를 검토하는 이유다.

UART 연결은 네이티브 USB PHY를 쓰지 않는다. 따라서 USB Host 방식에 필요한 PHY 전환은 없지만, 로그를 COM으로 옮기려면 여전히 앱 콘솔 설정을 별도로 바꿔야 한다. GPIO15/16의 최종 배정과 UART2 사용은 다른 주변장치의 점유 여부까지 확인한 뒤 확정한다. [ESP32 UART 문서][uart-doc] · [현재 Layer 8 소스](../layers/layer-8/main/main.c)
