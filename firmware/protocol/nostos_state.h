...
#define NOSTOS_REQUEST_CAPACITY 8U
...
typedef struct { nostos_report_t report; nostos_u16_value_t speed_kmh_x10; } nostos_speed_state_t;
typedef enum { NOSTOS_REAR_UNSEEN=0, NOSTOS_REAR_IS_SAFE, NOSTOS_REAR_IS_WARNING, NOSTOS_REAR_IS_UNKNOWN } nostos_rear_phase_t;
typedef struct { nostos_report_t report; nostos_rear_phase_t state; nostos_quality_t quality; } nostos_rear_state_t;
...
typedef struct {
    ...
    nostos_incident_state_t fall, sos;
...
typedef struct {
    ...
    nostos_message_t requests[NOSTOS_REQUEST_CAPACITY];
    uint32_t request_received_ms[NOSTOS_REQUEST_CAPACITY];
    size_t request_head, request_count;
    uint8_t local_source;
} nostos_receiver_t;
typedef enum { NOSTOS_LED_OFF=0, NOSTOS_LED_GREEN, NOSTOS_LED_YELLOW_BLINK, NOSTOS_LED_RED_BLINK } nostos_led_t;
typedef enum { NOSTOS_BUZZER_OFF=0, NOSTOS_BUZZER_REAR, NOSTOS_BUZZER_EMERGENCY } nostos_buzzer_t;
...
nostos_result_t nostos_receiver_pop_request(nostos_receiver_t *receiver, nostos_message_t *message);
