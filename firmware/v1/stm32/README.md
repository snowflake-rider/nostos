# NOSTOS Firmware v1 — STM32

NUCLEO-F411RE용 Prototype 완성본입니다. 버튼·USART1·RGB·VS1003B·부저·HC-SR04·MPU6050 캘리브레이션·SSD1306·DHT11을 한 디렉터리에 묶었습니다. 빌드 이름은 `nostos_stm32`입니다.

이 디렉터리는 `firmware/stm32-layer2/`에서 가져온 v1 스냅샷입니다. 공통 메시지 계약은 같은 묶음의 `../protocol/`을 사용합니다. `BUTTON_OUTPUT_TEST`가 아니라 기본 Prototype(v1) 구성이 기준입니다.
...
- 저장소 루트에서 `bash firmware/v1/stm32/tools/test-host.sh`: v1 STM32 전용 보드 없는 host-tests.
...
```sh
bash firmware/v1/stm32/tools/test-host.sh
```
...
`snowflake-rider/stm32-project`의 `940ff2408998d79e181e5b8322ba5e678c038871` 커밋에서 SSD1306·DHT11 드라이버를 가져오고, v1에 없는 `swarm_state` 의존성은 제외했습니다. 로컬 DHT11 측정값만 표시하는 서비스로 연결했습니다.

- SSD1306: I2C1 `PB8=SCL`, `PB9=SDA`, 기본 7-bit 주소 `0x3C`
- DHT11: `PA1`, 약 1.2초마다 측정
- 화면: 200ms마다 갱신하며 부팅 직후 `DHT WAIT`, 성공 시 온도·습도와 `DHT OK`, 실패 시 `DHT ERROR n` 표시
- SSD1306 초기화·갱신 실패 시 2초마다 재시도
- `SSD1306_DISPLAY`, `DHT11_SENSOR`는 기본 `ON`; `BUTTON_OUTPUT_TEST=ON`에서는 자동 `OFF`

MPU6050과 OLED는 같은 I2C1을 사용하지만 RTOS 없는 단일 메인 루프에서 순차 호출합니다. DHT11의 single-wire 타이밍 구간은 짧게 IRQ를 막으므로 실제 UART 부하에서 수신 누락 여부를 별도로 확인해야 합니다. 코드·호스트 테스트·ARM 빌드만으로 OLED 주소, 배선, DHT11 풀업과 센서 값을 증명할 수는 없습니다.

### 현재 v1 빌드 체크포인트

- host-tests: Debug/Release/Sanitized 각각 9/9 통과
- 설정: `NOSTOS_PROTOCOL_V2=OFF`, `BUTTON_OUTPUT_TEST=OFF`, `SSD1306_DISPLAY=ON`, `DHT11_SENSOR=ON`
- Arm GNU 15.3.1 `-Werror` Release: RAM 3,936 B, Flash/binary 137,632 B (`0x219A0`)
- binary SHA-256: `d76589510f81d40f09ac5a31a373481dca448e919d495a7a267f6620eaaf91b0`
- 세 보드 병렬 write는 macOS가 ST-LINK Debug 인터페이스에 device-access 권한을 요구해 write 시작 전에 중단됨
- 따라서 현재 세 보드는 아래의 화면 이식 전 이미지 상태이며, 새 화면 동작은 아직 실물 검증되지 않음

## 세 보드 플래시

Flash 주소는 STM32F411RE 내부 Flash 시작 주소 `0x08000000`입니다. 전체 chip erase, option byte, OTP는 변경하지 않습니다. `firmware/v1/build.sh stm32`가 Release ELF를 빌드하고 binary까지 변환합니다.
...
여러 ST-LINK가 동시에 연결되어 있을 때는 반드시 고유 시리얼을 지정합니다.
...
st-flash --serial 066DFF505567494867071811 --reset \
  write build/Release/nostos_stm32.bin 0x08000000
...
macOS에서 libusb device capture 권한 경고가 나오면 같은 명령을 `sudo`로 실행해야 합니다. 플래시 뒤에는 각 시리얼을 지정해 Flash를 다시 읽고 원본 binary와 비교하여 검증합니다. Virtual COM Port 이름은 다시 연결하면 달라질 수 있으므로 ST-LINK 고유 시리얼을 최종 기준으로 사용합니다.

```sh
st-flash --serial 066DFF485277504867161930 \
  read /tmp/v1-display-066D.bin 0x08000000 0x219A0
cmp build/Release/nostos_stm32.bin /tmp/v1-display-066D.bin

st-flash --serial 066DFF505567494867071811 \
  read /tmp/v1-display-066D-new.bin 0x08000000 0x219A0
cmp build/Release/nostos_stm32.bin /tmp/v1-display-066D-new.bin

st-flash --serial 066EFF3134584B3043121635 \
  read /tmp/v1-display-066E.bin 0x08000000 0x219A0
cmp build/Release/nostos_stm32.bin /tmp/v1-display-066E.bin
```

### 2026-08-29 화면 이식 전 플래시 기록

아래 기록은 SSD1306·DHT11 이식 전 이미지입니다. 화면 이식본과 크기·SHA-256이 다르며 새 이미지 배포 증거로 사용하지 않습니다.
...
- `066DFF505567494867071811`: write → 134,980 B read-back → `cmp`/SHA-256 일치 → reset
- `066EFF3134584B3043121635`: write → 134,980 B read-back → `cmp`/SHA-256 일치 → reset
- 마지막 USB 확인에서 세 대상 모두 다시 응답했고 세 read-back SHA-256이 동일
- 재부팅 직후 캘리브레이션 전 RGB OFF: 이전 두 보드에서 사용자 실물 확인 PASS; 새 세 번째 보드는 재확인 필요
...
공통 v2 codec·UART ISR ring·상태 적용·RGB/부저/오디오 경로를 추가했습니다. CMake `-DNOSTOS_PROTOCOL_V2=ON`으로 선택하며 기본은 기존 v1입니다. v2에서도 부저는 unmuted FALL 사건에만 허용하고 후방 경고·SOS에서는 끕니다. 신뢰된 세션/사건 복구를 담당하는 boot provider가 없으면 v2는 NOT_READY로 시작합니다. [구현 계약과 배포 경계](../protocol/V2.md) · [호스트 one-stop 테스트](../../../tests/message-protocol/README.md).
