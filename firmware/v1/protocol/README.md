> 이관 원문: `stm32-project/common/protocol/README.md`. 현재 실행 경로는 [팀원 시작 안내](../../../docs/getting-started/README.md)를 따른다. 아래의 과거 명칭·명령·검증 범위는 기록 당시 내용이다.

# API를 짧은 예제로 이해하기

[전체 흐름부터 보기](../../../docs/archive/legacy-stm32/integration/esp32-s3/README.md)
...
Mac에서 실행하려면 위 코드를 이 디렉터리의 `event_example.c`로 저장하고 다음을 실행합니다. C 컴파일러가 필요합니다.

```sh
cd firmware/v1/protocol
cc -std=c11 -I. event_example.c event_protocol.c -o event_example
./event_example
```
...
- 컴퓨터 예제와 달리 여러 Task가 같은 큐를 쓰면 보호가 필요합니다. 실제 ESP32에서는 이미 [bridge_runtime.c](https://github.com/snowflake-rider/nostos/blob/e234d90de76d4c750b77ae44f84182f0bc3e78a7/code/legacy/esp32-event-bridge/main/bridge_runtime.c)가 그 일을 합니다.
- 전송 함수의 성공은 **상대가 받았다는 뜻이 아닙니다**. 실제 시험에서는 상대 STM32의 수신도 확인해야 합니다.

8종 메시지 번호, 큐의 만료 규칙, callback 작성법은 [상세 참고](../../../docs/archive/legacy-stm32/integration/esp32-s3/DETAILS.md#공통-api-상세)에 남겨 두었습니다. 처음에는 위 예제만 이해하면 됩니다.

## v2 확장 프로토콜

[구현 계약](V2.md) · [모든 메시지 one-stop 테스트](../../../tests/message-protocol/README.md).
기존 event_protocol은 v1 기준을 보존하며, 새 nostos_* 모듈은 명시적으로 선택하는 v2 경로입니다.
