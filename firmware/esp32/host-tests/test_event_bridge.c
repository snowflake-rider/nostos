...
static void capacity_and_fifo(void)
{
    ...
    for (unsigned round = 0; round < 20; ++round) {
        ...
        CHECK(event_bridge_uart(&b, 0x13, 132, true) == EVENT_FULL);
        const uint8_t wire[] = {1, 0x13};
...
static void rejection_and_expiry(void)
{
    ...
    for (size_t i = 0; i < sizeof(removed_ids); ++i) {
        const uint8_t removed_wire[] = {1, removed_ids[i]};
        CHECK(event_bridge_uart(&b, removed_ids[i], 0, true) == EVENT_INVALID);
        CHECK(event_bridge_mesh(&b, removed_wire, 2, 2, 1, 0) == EVENT_INVALID);
    }
    ...
    event_stats_t s = event_bridge_stats(&b);
    CHECK(s.uart_noop == 1 && s.uart_invalid == 5 && s.not_ready == 2);
    CHECK(s.mesh_invalid == 8);
