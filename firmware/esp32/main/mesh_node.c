...
typedef struct {
    bool ready, subscribed;
    uint16_t primary, app, net, pub;
    uint8_t period, retransmit, ttl, relay;
} mesh_snapshot_t;
...
ESP_BLE_MESH_MODEL_PUB_DEFINE(
    event_pub, NOSTOS_APPLICATION_MAX_SIZE, ROLE_NODE);
...
static void refresh_snapshot(void)
{
    ...
    s.ready = s.primary != 0 && s.net != ESP_BLE_MESH_KEY_UNUSED &&
              ...
              s.relay == ESP_BLE_MESH_RELAY_ENABLED;
    portENTER_CRITICAL(&state_lock);
    snapshot = s;
    portEXIT_CRITICAL(&state_lock);
}
    ...
    size_t length)
{
    ...
    if (destination == 0U || wire == NULL ||
        nostos_message_decode(wire, length, &decoded) != NOSTOS_OK) {
    ...
    return esp_ble_mesh_server_model_send_msg(
        &vendor_models[0], &ctx, EVENT_OPCODE,
        (uint16_t)length, (uint8_t *)wire);
}
...
void mesh_node_log_status(void)
{
    ...
    ESP_LOGI(TAG, "STATUS name=%s primary=0x%04x event_ready=%u net=0x%04x app=0x%04x"
             " pub=0x%04x sub_C001=%u ttl=%u period=%u retransmit=%u relay=%u",
             device_name, s.primary, s.ready, s.net, s.app, s.pub, s.subscribed,
             s.ttl, s.period, s.retransmit, s.relay);
}
