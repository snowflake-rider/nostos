...
static const char *result_name(event_result_t result)
{
    switch (result) {
    case EVENT_OK: return "queued";
    case EVENT_EMPTY: return "empty";
    case EVENT_NOOP: return "noop";
    case EVENT_INVALID: return "invalid";
    case EVENT_SELF: return "self";
    case EVENT_NOT_READY: return "not_ready";
    case EVENT_FULL: return "full";
    case EVENT_EXPIRED: return "expired";
...
static void worker_task(void *argument)
{
    ...
    for (;;) {
        ...
        for (;;) {
            ...
            portEXIT_CRITICAL(&bridge_lock);
            ESP_LOGI(TAG, "%s id=0x%02x source=0x%04x api=%s age_ms=%" PRIu64,
                     job.direction == EVENT_TO_MESH ? "MESH_TX" : "UART_TX", job.id,
                     job.direction == EVENT_TO_MESH ? mesh_node_primary() : job.source,
    ...
    for (;;) {
        ...
        if (event.type == UART_DATA) {
            for (size_t i = 0; i < event.size; ++i) {
                ...
                /* 버튼 한 건이 UART에 도착했는지 보여 준다. 잡음/00은 카운터로만 확인한다. */
                if (result != EVENT_INVALID && result != EVENT_NOOP) {
                    ESP_LOGI(TAG, "UART_RX id=0x%02x result=%s", byte, result_name(result));
                }
            }
                            ...
                            uint16_t own_address)
{
    ...
    if (result == EVENT_OK) {
        ...
        /* wire는 callback 안에서만 읽는다. 큐에는 값이 이미 복사되어 있다. */
        ESP_LOGI(TAG, "MESH_RX source=0x%04x id=0x%02x result=queued", source, wire[1]);
    }
}
...
{
    uart_diag_log_status(DATA_UART, UART_TX_GPIO, UART_RX_GPIO);
    portENTER_CRITICAL(&bridge_lock);
