> 이관 원문: `layers/layer-8/README.md`. 현재 실행 경로는 [팀원 시작 안내](../../../docs/getting-started/README.md)를 따른다. 아래의 과거 명칭·명령·검증 범위는 기록 당시 내용이다.
...
현재 단일 버튼 실물 시험은 **STM32 D10/PB6 버튼 → USART1 D8/PA9 TX → ESP32 GPIO18** 구성이다. USART2는 ST-LINK USB 진단 복사본에만 사용한다. 배선과 자동 관찰 방법은 [빠른 버튼 → Mesh 검증](FAST_CHECK.md), 실제로 확인된 경계는 [STM32 버튼 시험 기록](../../../docs/verification/stm32-button-uart.md)을 따른다. USB 진단 복사본 수신만으로 D8에서 ESP32까지의 전기적 전달을 입증하지 않는다.
...
1. [main/main.c](../main/main.c): NVS → UART 입력 처리 → Bluetooth/Mesh → USB 콘솔.
2. [main/bridge_runtime.c](../main/bridge_runtime.c): UART 입력 Task, 공유 큐, 실제 송신 Task.
3. [main/mesh_node.c](../main/mesh_node.c): Provisioning/설정, Vendor 송수신, 기존 OnOff 모델.
4. [common/event_protocol.c](../../../libs/protocol/event_protocol.c): 1바이트 ID ↔ 2바이트 Mesh payload.
5. [common/event_bridge.c](../../../libs/protocol/event_bridge.c): 큐·만료·방향·자기 메시지 제외.
...
[Layers 목록](../../../docs/archive/layers/README.md) · [Layer 로드맵](../../../docs/archive/learning/LAYER-ROADMAP.md)
