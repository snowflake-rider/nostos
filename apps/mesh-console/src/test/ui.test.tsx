...
describe("logs", () => {
  it("searches Korean event labels and keeps raw text in details and exports", () => {
    const decoded = {
      ...rows[0],
      event: {
        id: 19,
        hex: "0x13",
        symbol: "MSG_STOP_REQUEST",
        label: "정지 요청",
        known: true,
        stage: "MESH_TX",
        source: "0x0005",
    ...
    render(<LogWorkspace logs={[decoded]} instance="decoded" />);
    fireEvent.change(screen.getByLabelText("로그 검색"), {
      target: { value: "정지 요청" },
    });
    const row = screen.getByRole("button", { name: /D6 MESH TX 정지 요청/ });
    fireEvent.click(row);
    ...
    expect(JSON.parse(serializeLogs([decoded], "jsonl"))).toEqual(decoded);
    expect(serializeLogs([decoded], "txt")).toContain(
      `[정지 요청 0x13] ${decoded.text}`,
    );
  });
  it("distinguishes automatic saved count, pending rows, and disk failure", () => {
    const storage = {
      enabled: true,
      directory: "/local/logs",
      file: "mesh-test.jsonl",
      saved: 7000,
      pending: 2,
      missed: 0,
      error: null as string | null,
    };
    const { rerender } = render(<StorageStatus storage={storage} />);
    expect(screen.getByText(/7,000줄 저장됨.*2줄 대기/)).toBeInTheDocument();
    rerender(
      <StorageStatus
        storage={{ ...storage, missed: 4, error: "로그 저장 실패" }}
      />,
    );
    expect(screen.getByRole("alert")).toHaveTextContent("미저장 4줄");
  });
