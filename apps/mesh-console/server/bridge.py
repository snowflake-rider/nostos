...
from .log_store import LOG_ROOT, LogStore
...
class Bridge:
    def __init__(self, scanner=list_ports.comports, opener=open_serial, mode="live", log_directory=None):
        ...
        directory = log_directory if log_directory is not None else LOG_ROOT / "live" if mode == "live" else None
        self.archive = LogStore(directory, self.instance, mode)
    ...
    def snapshot(self):
        ...
        return {"type": "state", "instance": self.instance, "mode": self.mode, "nodes": nodes,
                ...
                "poll_seconds": 5, "limit": LOG_LIMIT, "storage": self.archive.snapshot()}
    ...
    def log(self, board, text, direction="rx", level=None):
        ...
        row = {"id": self.seq, "time": datetime.now(timezone.utc).isoformat(timespec="milliseconds"),
               ...
               "event": parsed["event"] if direction == "rx" else None}
