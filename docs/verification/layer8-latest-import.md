...
기록일: **2026-08-28**. [사용 방법](../../firmware/esp32/docs/layer8-background.md).

최신 B6 재등록·수신 결과는 바로 아래 절을 따른다. 이후 절들은 각 시험 당시의
기록이며, 옛 USART2 배선과 옛 이미지 해시를 현재 설정으로 사용하지 않는다.

## 최신 — 2026-08-28 B6 재등록 및 이벤트 수신 확인

사용자가 nRF Mesh에서 기존 네트워크에 B6를 다시 등록했다. 옛 주소 `0x0004`의
수동 재사용은 앱에서 거부됐으며, 앱이 제안한 새 주소 `0x0006`을 사용했다.
앱의 거부 원인 자체는 확인하지 않았다. `events` AppKey index 1을 Add/Bind하고
Vendor Model의 C001 Publication/Subscription을 설정했다.

| 노드 | 실제 primary | net / app | event_ready / sub_C001 | pub / TTL / period / retransmit |
| --- | --- | --- | --- | --- |
| B6 | `0x0006` | `0x0000` / `0x0001` | 1 / 1 | C001 / 7 / 0 / 0 |
| D6 | `0x0005` | `0x0000` / `0x0001` | 1 / 1 | C001 / 7 / 0 / 0 |
| 76 | `0x0003` | `0x0000` / `0x0001` | 1 / 1 | C001 / 7 / 0 / 0 |

첫 버튼 4는 D6 UART_RX/MESH_TX 및 76 MESH_RX/UART_TX만 관찰됐고 B6 수신은 0이었다.
후속 버튼 4에서는 **B6의 `MESH_RX source=0x0005 id=0x13 result=queued`와
`UART_TX id=0x13 source=0x0005 api=accepted`**까지 확인했다. 첫 누락 원인은 확정하지 않았다.

사용자가 4→3→2→1을 눌렀다고 보고한 뒤 읽기 전용 status 결과:
D6 UART_RX valid=5/MESH_TX accepted=5, 76 MESH_RX valid=5/UART_TX accepted=5,
B6 MESH_RX valid=4/UART_TX accepted=4. 관련 invalid/failed=0, 모든 pending=0.
첫 시험을 포함한 누적값이다. 관찰기가 두 번째 버튼 4 수신 직후 종료되어
3·2·1의 개별 ID/순서는 캡처하지 못했다. 네 건의 누적 수신과 개별 ID 확인은 구분한다.

**확인 범위:** D6발 버튼 이벤트의 B6 Mesh 수신 및 UART 송신 API 성공.
B6발 역방향, 상대 STM32 실물 출력, 재부팅 후 유지, 장시간 무손실 전달은 이번 결과에 포함하지 않는다.
이번 단계에서 도구는 USB `status`와 로그 읽기만 사용했고, flash/erase/reset/이벤트 주입은 하지 않았다.
원시 로그와 결과 (원본에 포함되지 않은 참조: `../../build/hardware-results/b6-rejoin-8vnbdmzz/RESULT.md`).

## 이전 — 2026-08-28 B6 전체 삭제 후 Layer 8 설치

사용자가 추가 백업 없이 전체 삭제 후 새 설치를 명시적으로 요청했다.
USB serial `44:1B:F6:FF:BA:B4`인 B6만 대상으로 실행했다. 당시 포트는
`/dev/cu.usbmodem1201`이며, 포트 번호가 아닌 USB serial로 식별했다.

- 현재 소스로 `idf.py build` 성공. 앱 크기 915856 bytes.
- `erase_flash` 성공: 기존 NVS/Mesh 등록 정보도 삭제. 이전 설정 복원 없음.
- bootloader `0x0`, partition table `0x8000`, app `0x10000` 설치.
- 세 이미지 모두 쓰기 hash 검증 및 별도 `verify_flash` digest 검증 통과.
- 첫 reset 후 USB 무응답 관찰. 이후 B6에 `--before usb_reset --after hard_reset read_mac`을
  실행한 뒤 실제 `BOOT_START`, `APP_STARTED`, USB `status` 응답 확인.
- 실행 프로젝트 `esp32s3_layer_8`, ESP-IDF v5.5.5, ELF SHA prefix `07e1dedd3`.
- 이름 `ESP32-L8-BAB6`, primary=0x0000, event_ready=0, sub_C001=0: **미등록 상태**.
- UART1 진단 TX=GPIO17, RX=GPIO18, baud=115201(설정 115200), 8N1, flow=0.
- 두 status 표본에서 uart_rx valid/noop/invalid/hw_errors=0, pending=0.
  UART 입력 레벨은 LOW로 관찰되었으며, 데이터 전달 성공으로 판정하지 않았다.
- D6·76·STM32 erase/flash/reset 없음. 낙상 이벤트 주입/실물 시험 없음.

설치 직후에는 B6 재등록과 AppKey/model/group 설정이 필요했다. 그 후속 결과는 위 최신 절을 따른다.
옛 B6 주소/키/준비 완료 상태를 현재 상태로 사용하지 않는다.
이번 설치 증거 (원본에 포함되지 않은 참조: `../../build/hardware-results/b6-layer8-install-1189fty3/RESULT.md`).

## 이전 — 2026-08-28 D6/76 UART 진단 앱 설치
...
사용법과 해석 한계: [../../firmware/esp32/docs/UART_DIAGNOSTICS.md](../../firmware/esp32/docs/UART_DIAGNOSTICS.md).
...
빠른 버튼 관찰 도구는 [FAST_CHECK](../../firmware/esp32/docs/FAST_CHECK.md)를 따른다. 2026-08-28 문서 갱신 중 다음 호스트 회귀 검사를 다시 실행해 14/14 PASS를 확인했다.
...
- EC74 백업·Flash 로그 (원본에 포함되지 않은 참조: `logs/flash-EC74-20260828T102023-KST.log`)
- BAB4 백업·Flash 로그 (원본에 포함되지 않은 참조: `logs/flash-BAB4-20260828T102023-KST.log`)
- 두 대 부팅·status 응답 로그 (원본에 포함되지 않은 참조: `logs/boot-status-pair-20260828T102023-KST.log`)
...
- D6 백업·Flash 로그 (원본에 포함되지 않은 참조: `logs/flash-D6-20260828T112007-KST.log`)
...
- [STM32 버튼·USART2 설치와 실제 관찰](../archive/imported/stm32-project/integration/stm32/BUTTON_UART_TEST.md)
