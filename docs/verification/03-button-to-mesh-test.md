> 이관 원문: `verified/03-button-to-mesh-test.md`. 현재 실행 경로는 [팀원 시작 안내](../getting-started/README.md)를 따른다. 아래의 과거 명칭·명령·검증 범위는 기록 당시 내용이다.
...
1. [button.c](../../firmware/stm32/MyApp/hw/button.c): D10/PB6 눌림 전이를 `MSG_STOP_REQUEST`로 만든다.
2. [app.c](../../firmware/stm32/MyApp/ap/app.c): 버튼 메시지를 로컬 publish 경로에 넘긴다.
3. [uart_service.c](../../firmware/stm32/MyApp/service/uart_service.c): USART1로 한 바이트 송신하고, HAL 성공 시 USART2에 진단 복사본을 출력한다.
4. [bridge_runtime.c](../../firmware/esp32/main/bridge_runtime.c): D6 UART 입력을 Mesh로 넘기고, 76의 Mesh 수신 콜백에서 source·ID·queued 결과를 기록한다.
