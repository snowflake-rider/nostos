> 이관 원문: `docs/02-learning/03-ble-advertising.md`. 현재 실행 경로는 [팀원 시작 안내](../00-team/START.md)를 따른다. 아래의 과거 명칭·명령·검증 범위는 기록 당시 내용이다.
...
- [sdkconfig.defaults](../../code/layers/layer-1/sdkconfig.defaults): Bluetooth와 Bluedroid를 켜는 초기 설정.
- [main/CMakeLists.txt](../../code/layers/layer-1/main/CMakeLists.txt): `main.c`와 필요한 `bt`, `nvs_flash` 컴포넌트를 지정.

Layer 1에는 이미 준비되어 있다. 실제 빌드는 저장된 `sdkconfig`를 사용하며, 나중에 defaults만 바꿔도 기존 설정이 자동으로 덮어써지지는 않는다.

## 2. 초기화와 Advertising 코드 살펴보기

[main/main.c](../../code/layers/layer-1/main/main.c)는 다음 순서로 동작한다. 지금은 함수 세부 구현보다 순서를 이해한다.
