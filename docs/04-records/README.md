> 이관 원문: `docs/04-records/README.md`. 현재 실행 경로는 [팀원 시작 안내](../00-team/START.md)를 따른다. 아래의 과거 명칭·명령·검증 범위는 기록 당시 내용이다.

# 학습·설계 기록

[전체 시작 메뉴](esp-ble-original-index.md) · [학습 순서](../02-learning/README.md) · [현재 진행 상태](../01-project/STATUS.md)
...
| [STM32 버튼/UART 시험](../firmware/stm32/BUTTON_UART_TEST.md) | PB6 → USART2 → ST-LINK VCP까지 확인하고 외부 D1 → ESP32 구간을 미검증으로 분리 |
| [STM32 Event bridge 검증](../legacy-stm32/integration/esp32-s3/VERIFICATION.md) | 공통 codec/queue, ESP32·STM32 build와 남은 실제 종단 간 시험 |
| [STM32 SharedState](../firmware/stm32/SHARED_STATE.md) | A/B/C 센서값 저장·조회 모듈과 실제 UART·Dashboard 연결 경계 |
