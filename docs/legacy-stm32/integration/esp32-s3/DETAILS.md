> 이관 원문: `stm32-project/integration/esp32-s3/DETAILS.md`. 현재 실행 경로는 [팀원 시작 안내](../../../00-team/START.md)를 따른다. 아래의 과거 명칭·명령·검증 범위는 기록 당시 내용이다.
...
ID의 유일한 원본은 [message_type.h](../../../../code/common/protocol/message_type.h)다. CMake include 경로로 양쪽에서 참조하며 enum을 복제하지 않는다.
...
1. [event_protocol.h](../../../../code/common/protocol/event_protocol.h): `event_id_valid`, `event_encode`, `event_decode`.
2. [event_bridge.h](../../../../code/common/protocol/event_bridge.h): 입력을 값 복사해 넣고, 만료·준비 상태를 검사하며 꺼내는 고정 FIFO.
3. [event_bridge.c](../../../../code/common/protocol/event_bridge.c): 실제 정책. [tests](../../../../code/common/protocol/tests)에서 공개 API의 예제를 확인한다.
...
이 C 객체 자체는 thread-safe가 아니다. ESP32 [bridge_runtime.c](../../../../code/legacy/esp32-event-bridge/main/bridge_runtime.c)가 **모든 큐·통계 접근을 같은 `portMUX`로 보호**한다. UART Task와 Mesh callback은 짧은 critical section 안에서 값 복사만 하고, Bridge Task는 잠금을 푼 뒤 전송한다. Task notification은 깨우기 신호일 뿐이며 이벤트 저장은 FIFO가 담당한다. 다른 RTOS에 포팅할 때도 같은 직렬화가 필요하다.
