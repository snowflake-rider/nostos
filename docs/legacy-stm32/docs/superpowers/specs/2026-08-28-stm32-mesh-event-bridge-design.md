> 이관 원문: `stm32-project/docs/superpowers/specs/2026-08-28-stm32-mesh-event-bridge-design.md`. 현재 실행 경로는 [팀원 시작 안내](../../../../00-team/START.md)를 따른다. 아래의 과거 명칭·명령·검증 범위는 기록 당시 내용이다.
...
상태: **상세 계약 사용자 승인 후 1차 소스 구현·호스트 검사·양쪽 MCU 빌드 완료. 실물 통합은 미검증.** 현재 사용법은 [ESP32 README](../../../integration/esp32-s3/README.md), 증거와 미확인 항목은 [검증 기록](../../../integration/esp32-s3/VERIFICATION.md)을 따른다.
...
- [message_type.h](../../../../../code/common/protocol/message_type.h): 공통 이벤트 ID의 유일한 원본. HAL에 의존하지 않으므로 STM32/ESP32/호스트 코드에서 같은 헤더를 참조한다.
- [uart_service.c](../../../../../code/firmware/stm32/MyApp/service/uart_service.c): USART1으로 ID 한 바이트를 송수신한다. 현재 수신 대기 공간은 한 건이다.
- [message_router.c](../../../../../code/firmware/stm32/MyApp/service/message_router.c): 로컬 이벤트는 로컬 처리 후 송신하고, 원격 이벤트는 로컬 처리만 한다.
...
- Bridge Task가 앱의 송신 작업을 소유한다. 정적 FIFO 32건으로 UART→Mesh / Mesh→UART 작업을 처리하며, ESP32 adapter가 모든 큐·통계 접근을 같은 `portMUX`로 보호한다. 입력 거부 카운터는 생산자가 같은 보호 아래 갱신한다. Task notification은 깨우기 용도다. 같은 ID의 서로 다른 입력은 별도 작업이다.
...
- UART 출력은 한 바이트이며 TX 완료 대기 예산 10ms를 둔다. Bridge Task만 UART1 TX를 소유하고 드라이버가 하드웨어 FIFO에 값을 복사한 후 반환한다. RTOS 스케줄링을 포함한 hard real-time 전달 상한은 보장하지 않는다.
