> 이관 원문: `docs/03-reference/ESP-BLE-MESH-C3.md`. 현재 실행 경로는 [팀원 시작 안내](../../getting-started/README.md)를 따른다. 아래의 과거 명칭·명령·검증 범위는 기록 당시 내용이다.
...
이 문서는 **ESP32-C3용 참고 자료**다. 현재 ESP32-S3 실습은 [Layer 로드맵](../learning/LAYER-ROADMAP.md)을, 보드 검증 상태는 [진행 기록](../project/STATUS.md)을 본다. C3 자료의 빌드·Flash 상태와 S3 결과를 섞지 않는다.
...
![ESP-BLE-MESH 펌웨어를 올리고 전원을 켠 세 보드](../../assets/ble-mesh-device-power-on.png)
...
![nRF Mesh 앱에서 검색된 Unprovisioned Device](../../assets/ble-mesh-scanner.png)
...
![장치 식별과 Provision 버튼](../../assets/ble-mesh-identify-provision.png)
...
![Provisioning과 초기 설정이 모두 완료된 화면](../../assets/ble-mesh-config-complete.png)
...
![Provisioning 후 초기 설정이 끝나지 않은 상태](../../assets/ble-mesh-initial-config-fail.png)
...
![재연결 후 Composition Data와 AppKey 설정 화면](../../assets/ble-mesh-reconnect-initial-config.png)

여러 Node를 Provisioning했다면 재연결 목록에 두세 개의 Node가 함께 표시될 수 있다. 연결 후 Node별 Composition Data 상태를 확인한다.

![세 Node 중 초기 설정이 완료되지 않은 Node 확인](../../assets/ble-mesh-reconnect-three.png)
...
![Generic OnOff Server Model에 AppKey를 Bind하는 화면](../../assets/ble-mesh-model-bind-appkey.png)
...
![nRF Mesh 앱의 Generic OnOff 제어 화면](../../assets/ble-mesh-generic-onoff.png)

앱에서 On/Off 명령을 보내 다른 보드의 LED가 바뀌는지 확인한다.

![서로 다른 RGB LED 상태를 표시하는 세 Mesh Node](../../assets/ble-mesh-three-nodes-on.png)
