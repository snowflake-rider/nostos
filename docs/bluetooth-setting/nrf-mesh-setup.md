...
> 일반적인 3노드 설정은 [Bluetooth Mesh 빠른 설정](README.md)만 따르면 된다.
> 이 문서는 키 불일치, 재등록, 앱 데이터 복원처럼 문제가 있을 때 사용하는 상세 참고 자료다.
...
첫 시험 누락과 확인 범위는 재등록 결과 (원본에 포함되지 않은 참조: `../build/hardware-results/b6-rejoin-8vnbdmzz/RESULT.md`)를 따른다.
...
기존 [Layer 7 매뉴얼](../hardware/how-to-make-mesh-network.md)의 C000 설정만으로 버튼·낙상 이벤트가 준비되지는 않는다.
Vendor Company ID `0x02E5`는 현재 학습용 구현의 값이며 제품용 할당 ID 안내는 아니다.

설정값 근거: [mesh_node.h](../../firmware/esp32/main/mesh_node.h),
[mesh_node.c의 composition·refresh_snapshot](../../firmware/esp32/main/mesh_node.c).
...
관련 설치 기록: [Layer 8 검증](../verification/layer8-latest-import.md).
...
5. 완전한 신규 구성이라면 `Settings → Application Keys → +`에서 공용 AppKey를 **한 번만** 만든다. 현재 Export의 이름은 `Events`다. Bound Network Key는 사용할 NetKey로 지정한다.
...
상태 기록: [testing/checklist.md](../archive/imported/testing/checklist.md), 배선: [wiring.md](../hardware/wiring.md).
...
과거 [B6 키 불일치 진단](B6_SETUP.md)은 이전 시점의 기록이다.
...
이 `bluetooth-setting/`에는 절차만 보관하고 실제 네트워크 JSON·키 원문은 저장하지 않는다.
...
- [Layer 8 설정·코드 설명](../../firmware/esp32/docs/layer8-background.md): 모델·프로토콜 참고. 예전 단일 버튼 설명과 현재 버튼 배선은 구분한다.
- [읽기 전용 UART 진단](../../firmware/esp32/docs/UART_DIAGNOSTICS.md): `status` 의미와 물리 전달 검증의 한계.
