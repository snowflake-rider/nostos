...
[프로젝트](../../README.md) · [문서 목록](../README.md) · [이번 이관·검사 결과](../archive/restructure/README.md)
...
전원 공통 기능은 버튼, LED, 부저, VS1003B 음성 출력, UART·Mesh 통신입니다. 각 담당자는 기존 [MyApp](../../firmware/stm32/MyApp) 안의 자기 기능을 수정합니다.
...
[app_config.h](../../firmware/stm32/MyApp/common/app_config.h)의 현재 기본값은 `FEATURE_ULTRASONIC_SENSOR=1`, `FEATURE_FALL_DETECTION=1`입니다. 이관에서는 값을 바꾸지 않았습니다. 장착 센서에 맞게 기능 플래그를 확인하고 빌드합니다. 센서 수신 알림과 로컬 센서 측정은 별개이므로 센서를 끄더라도 메시지 수신·출력 코드는 제거하지 않습니다.
...
CubeMX 프로젝트는 [nostos_stm32.ioc](../../firmware/stm32/nostos_stm32.ioc)입니다. STM32CubeMX에서 코드를 재생성하는 작업은 이번 이관에서 수행하지 않았습니다. 기존 `Core`, `Drivers`, startup, linker script와 `MyApp`을 사용합니다.

## 3. ESP32 빌드

팀 배포 및 기능 확장 기준은 **기존 Layer 8에서 가져온** [firmware/esp32](../../firmware/esp32) 하나입니다. 학습용 layers, 이전 통합 ESP32, 원본 사본 폴더는 현재 작업 트리에서 제거했으며 Git 이력과 외부 백업에 보존했습니다.
...
USB 콘솔 모니터는 저장소 루트에서 `ESP_PORT=<실제 포트> bash tools/monitor.sh`로 실행합니다. 현재 ESP32 펌웨어를 대상으로 하며, 사용법만 확인하려면 `bash tools/monitor.sh --help`를 실행합니다. 실제 모니터는 USB 포트를 여므로 다른 모니터를 먼저 종료합니다.
...
현재 단방향 검증 배선의 핵심은 다음입니다. 전원을 끄고 [상세 핀 안내](../hardware/wiring.md)를 확인합니다.
...
검증 도구는 [fast-check 설명](../../firmware/esp32/docs/FAST_CHECK.md)을 읽고 사용합니다. 새 위치는 `firmware/esp32/fast-check.sh`입니다. 도구의 장치 ID와 기대 수신기 목록은 기존 시험 장비 기준이므로 팀원의 보드·현재 연결 대수에 맞는지 먼저 확인해야 합니다. 이번 이관에서는 도구를 실제 serial port에 연결하지 않았습니다.

`MESH_TX api=accepted`는 상대 수신 보장이 아닙니다. [기존 Verified](../verification/README.md)는 D6·76을 사용한 당시 단방향 시험이며, 세 노드 controlled relay나 상대 STM32 출력 검증까지 뜻하지 않습니다.
...
```sh
cmake -S libs/protocol -B libs/protocol/build -G Ninja
cmake --build libs/protocol/build
ctest --test-dir libs/protocol/build --output-on-failure
```

ESP32 쪽 기존 Layer 8 호스트 검사는 `bash firmware/esp32/test-host.sh`로 실행합니다. 문서·저장소 구조 검사는 `python3 tools/check_repository.py`로 실행합니다. 둘 다 보드 Flash를 하지 않습니다.
