...
상태: 이름, 대상 저장소, 코드/문서 분리는 사용자 승인 완료. 아래 구체적인 경로 배치는 이관 전 검토 대상이다. 이 문서 작성만으로 코드 이관·빌드·업로드가 완료된 것은 아니다.
...
현재 `integration/esp32-s3`와 `layers/layer-8`은 동일하지 않다. 예를 들어 통합 원본에는 `mesh_node_send_onofSf` 오타가 있고 Layer 8에는 수정과 UART 진단 코드가 있다.

이관에서 두 버전을 몰래 합치지 않는다. 팀 통합 펌웨어는 기존 통합 원본에서 시작하고, 빌드를 막는 명백한 오타·이동 경로 문제만 별도 변경으로 기록하여 수정한다. Layer 8은 독립적으로 보존한다. 실물 확인 기록은 Layer 8 및 당시 STM32 소스의 증거이며, 새 `nostos_esp32`의 실물 검증으로 승계하지 않는다. 기능 차이를 통합하는 작업은 후속 단계로 둔다.
...
이 문서의 경로 배치를 사용자가 검토한 뒤 이관 구현 계획을 만들고 실행한다. 현재 단계는 저장소 연결과 설계 문서 준비이며 실제 코드 이관과 새 이름의 빌드는 아직 실행하지 않았다.
