> 이관 원문: `communication-module/periodic/README.md`. 현재 실행 경로는 [팀원 시작 안내](../../../docs/getting-started/README.md)를 따른다. 아래의 과거 명칭·명령·검증 범위는 기록 당시 내용이다.
...
- [moving_average.h](moving_average.h), [moving_average.c](moving_average.c): 최근 5개 유한 측정값과 평균. 5개 전에는 평균을 제공하지 않는다.
- [comm_periodic.h](comm_periodic.h), [comm_periodic.c](comm_periodic.c): 속도 입력, 유효 여부, 만료, 전송 시점 판단.
- [가짜 입력 예제](examples/main.c): 처음 값 없음 → 5개 준비 → BUSY/재시도 → 만료 → 유효한 정지값.
- [이동평균 테스트](tests/test_moving_average.c), [주기 전송 테스트](tests/test_periodic.c): 공개 API를 통한 검증.
