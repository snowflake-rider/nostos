...
static void execute_command(serial_command_t command, uint16_t source) {
  ...
  case SERIAL_COMMAND_STATUS:
    ...
    gps_mesh_log_status();
    return;
  case SERIAL_COMMAND_GPS_SOURCE:
    err = gps_mesh_set_source(source);
    break;
  case SERIAL_COMMAND_FACTORY_RESET:
    ESP_LOGE(TAG, "FACTORY_RESET_DISABLED");
    return;
...
static void serial_command_task(void *argument) {
  ...
  ESP_LOGI(TAG,
           "[LAYER-7] SERIAL_READY commands=on,off,on-unack,off-unack,tx-low,tx-normal,status,gps-source 0xNNNN");
  ...
  while (true) {
    ...
    if (serial_command_parser_feed(&parser, (uint8_t)byte, &command)) {
      execute_command(command, parser.gps_source);
    }
  }
}
...
void app_main(void) {
  ...
  ESP_LOGI(TAG, "[GPS_DEMO] firmware=gps-mesh-node protocol=1 vendor=02E5:1001");
  ...
  err = nvs_flash_init();
  if (err != ESP_OK) {
    ESP_LOGE(TAG, "NVS_INIT_FAILED err=%s; stopped without erasing network", esp_err_to_name(err));
    return;
  }
