/* 김현수 제출 코드 기반. 원문: originals/day06.md */
#include "rotate.h"
 ...
 * - **Day 6. 원형 비트 시프트 (Circular Shift/Rotate)**
    ...
    ```
 */
static void test_lr(uint32_t n, unsigned int steps);

static void print_binary(uint32_t n);
uint32_t rp06_rotate(uint32_t n, unsigned int steps, bool dir);
...
static void test_lr(uint32_t n, unsigned int steps)
{
    ...
    print_binary(n);
    // Rotate Left
    uint32_t result = rp06_rotate(n, steps, 0);
    printf("\n\n[ROL %u]  Hex: 0x%08X\n", steps % 32U, result);
    printf("         Bin: "); // 9 spaces
    print_binary(result);
    printf("\n         (MSB bits moved to LSB)");
    ...
    // Rotate Right
    result = rp06_rotate(result, steps, 1);
    printf("\n\n[ROR %u]  Hex: 0x%08X\n", steps % 32U, result);
    printf("         Bin: "); // 9 spaces
    print_binary(result);
    printf("\n         (Restored to original)");
