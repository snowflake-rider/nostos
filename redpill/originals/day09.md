...
출처: https://app.notion.com/6fa2ae70087183c0a547010b3926cb5c
 ...
 */
typedef void (*PairPrinter)(const void *a, const void *b);

// HELPERS
static void my_swap(void *a, void *b, size_t size);
static void print_pair(const char *type_name, const char *stage, PairPrinter printer, const void *a, const void *b);
...
int main(void)
{
    ...
    my_swap(&int_a, &int_b, sizeof int_a);
    ...
    my_swap(&db_a, &db_b, sizeof db_a);
    print_pair("Double", "After", print_double_pair, &db_a, &db_b);
    putchar('\n');

    // CASE: STRUCT PERSON
    Person ps_a = {.lname = "Kim", .id = 1}, ps_b = {.lname = "Lee", .id = 2};
    print_pair("Struct", "Before", print_person_pair, &ps_a, &ps_b);
    my_swap(&ps_a, &ps_b, sizeof ps_a);
