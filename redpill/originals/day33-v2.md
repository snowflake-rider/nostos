...
static uint8_t usart2_input_size = 0;
static uint8_t *usart2_data;
static bool usart2_writing = false;

uint8_t usart1_rx_buf[RX_BUF_SIZE];
uint8_t usart2_rx_buf[RX_BUF_SIZE];
static uint8_t chat_buffer[RX_BUF_SIZE];
static uint16_t chat_length = 0;
static const uint8_t newline[] = "\r\n";
// erase moves the space back and erase the next char and move back.
static const uint8_t erase[] = "\b \b";

static void collect_input(const uint8_t *data, uint8_t size);
static void collect_input(const uint8_t *data, uint8_t size) {
  for (uint16_t i = 0; i < size; ++i) {
    ...
    // 2. \r or \n => SEND Message
    if (ch == '\r' || ch == '\n') {
      // Ignore carriage return or new line
      if (chat_length == 0) {
        continue;
      }
      chat_buffer[chat_length] = '\0';
      // Send Message
      HAL_UART_Transmit(&huart1, chat_buffer, chat_length, TIME_OUT);
      // Send New Line
      HAL_UART_Transmit(&huart1, newline, 2, TIME_OUT);

      // Display New Line in my Terminal
      HAL_UART_Transmit(&huart2, newline, 2, TIME_OUT);

      // Reset the chat_buffer
      memset(chat_buffer, 0,
             sizeof(chat_buffer)); // added for genuine buffer clearing
    ...
    if (ch == '\b' || ch == 0x7F) {
      if (chat_length > 0) {
        ...
        HAL_UART_Transmit(&huart2, erase, 3, 100);
        chat_buffer[chat_length] = '\0';
        chat_length--;
      }
      continue;
    }
    // put ch in chat_buffer
    if (chat_length < RX_BUF_SIZE - 1) {
      chat_buffer[chat_length++] = ch;
      // display chars in my terminal
...
/**
⭐ TERMINAL
tio --baudrate 115200 /dev/cu.usbmodem1102

Control-T, then e — toggle local echo
Control-T, then c — show current configuration
Control-T, then q — quit
Control-T, then ? — show all commands
 */
...
void HAL_UARTEx_RxEventCallback(UART_HandleTypeDef *huart, uint16_t Size) {
  if (huart->Instance == USART1) {
    ...
    // ✅ change transmit to custom function
    // HAL_UART_Transmit(&huart1, usart2_rx_buf, Size, TIME_OUT);
    collect_input(usart2_rx_buf, Size); //
    // // 1. Flag up
    // usart2_writing = true;
    // usart2_input_size = Size;
    // usart2_data = usart2_rx_buf;
