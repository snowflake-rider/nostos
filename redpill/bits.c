/* 김현수 제출 코드 기반. 원문: originals/day04.md */
#include "bits.h"
...
uint8_t rp04_reverse_bits(uint8_t x);
static void print_bits(uint8_t value);
static void run_case(unsigned int case_number, uint8_t input, uint8_t expected);
...
static void run_case(unsigned int case_number, uint8_t input, uint8_t expected) {
  uint8_t output = rp04_reverse_bits(input);
