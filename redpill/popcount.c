/* 김현수 제출 코드 기반. 원문: originals/day05.md */
#include "popcount.h"
...
static void print_case(unsigned int case_number, uint32_t input);
...
unsigned int rp05_popcount_builtin(uint32_t n)
{
  ...
  return (unsigned int)__builtin_popcountll((unsigned long long)n);
#else
  return rp05_popcount_swar(n);
#endif
}
...
static void print_case(unsigned int case_number, uint32_t input)
{
  ...
  printf("  [Naive]     : %u\n", rp05_popcount_naive(input));
  printf("  [Kernighan] : %u (Recommended Logic)\n", rp05_popcount_brian(input));
  printf("  [SWAR]      : %u (Strict Loop-free)\n", rp05_popcount_swar(input));
  printf("  [Built-in]  : %u\n", rp05_popcount_builtin(input));
  printf("------------------------------\n");
}
