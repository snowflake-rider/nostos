...
LOG_FILE="$LOG_DIR/esp32s3-layer-0-bootload-$RUN_TIMESTAMP.log"
...
printf 'ESP32-S3 Layer 0 bootloading workflow\n'
...
printf 'Waiting up to %s seconds for [LAYER-0] RUNTIME_OK...\n' \
  "$MONITOR_TIMEOUT_SECONDS"

if ! python - "$PORT" "$MONITOR_BAUD" "$MONITOR_TIMEOUT_SECONDS" <<'PY'
import sys
import time
...
marker = "[LAYER-0] RUNTIME_OK"
deadline = time.monotonic() + timeout_seconds

try:
    with serial.Serial(port, baudrate=baud, timeout=0.25) as device:
        # DTR/RTS를 inactive로 두어 application 실행 상태를 유지한다.
        device.dtr = False
        device.rts = False
