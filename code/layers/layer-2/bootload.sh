...
finish() {
  ...
  if [[ "$RESULT" == "PASS" && $exit_code -eq 0 ]]; then
    ...
    printf 'PHONE_GATT_TEST=NOT_VERIFIED\n'
    printf 'PHONE_CONNECT_TARGET=ESP32-LAYER-2\n'
    printf 'RX_WRITE_TEST=HELLO\n'
    printf 'TX_NOTIFY_EXPECTED=ACK:HELLO\n'
  else
    if ((exit_code == 0)); then
      exit_code=1
    fi
...
printf 'ESP32-S3 Layer 2 BLE GATT Server workflow\n'
