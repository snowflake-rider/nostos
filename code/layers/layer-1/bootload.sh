...
finish() {
  ...
  if [[ "$RESULT" == "PASS" && $exit_code -eq 0 ]]; then
    ...
    printf 'OVER_AIR_SCAN=NOT_VERIFIED\n'
    printf 'PHONE_SCAN_TARGET=ESP32-LAYER-1\n'
  else
    if ((exit_code == 0)); then
      exit_code=1
    fi
...
printf 'ESP32-S3 Layer 1 BLE Advertising workflow\n'
...
command -v idf.py >/dev/null 2>&1 || fail 'idf.py is unavailable after ESP-IDF export.'
command -v esptool.py >/dev/null 2>&1 || fail 'esptool.py is unavailable after ESP-IDF export.'
command -v python >/dev/null 2>&1 || fail 'ESP-IDF Python interpreter is unavailable.'
...
printf '%s\n' "$flash_output"
printf '%s\n' "$flash_output" | grep -Eiq "Detected flash size:[[:space:]]*$EXPECTED_FLASH_SIZE" || \
...
if ! python - "$PORT" "$MONITOR_BAUD" "$MONITOR_TIMEOUT_SECONDS" "$RUNTIME_MARKER" <<'PY'
import sys
import time
...
try:
    with serial.Serial(port, baudrate=baud, timeout=0.25) as device:
        ...
        while time.monotonic() < deadline:
            ...
            if marker in line:
                print("RUNTIME_MARKER_FOUND=yes", flush=True)
                raise SystemExit(0)
except serial.SerialException as error:
    print(f"SERIAL_ERROR={error}", file=sys.stderr, flush=True)
    raise SystemExit(2)

print("RUNTIME_MARKER_FOUND=no", file=sys.stderr, flush=True)
raise SystemExit(1)
PY
then
  fail "Runtime marker was not received from $PORT."
fi
