...
usage() {
  printf '%s\n' \
    ...
    '' \
    'This verifies one board local runtime. Use bootload-pair.sh for direct RX and forward TX.' \
    '' \
    'Environment:' \
...
finish() {
  ...
  if [[ "$RESULT" == "PASS" && $exit_code -eq 0 ]]; then
    ...
    printf 'LOCAL_ORIGIN_TX=PASS\n'
    printf 'PAIR_DIRECT_RX=NOT_VERIFIED\n'
    printf 'PAIR_FORWARD_TX=NOT_VERIFIED\n'
...
printf 'ESP32-S3 Layer 6 one-board relay-node workflow\n'
    ...
    with serial.Serial(port, baudrate=baud, timeout=0.25) as device:
        ...
        while time.monotonic() < deadline:
            ...
            if marker in line and "gatt=yes" in line and \
                ...
                runtime_ready = True
            if "[LAYER-6] PACKET_TX_ORIGIN " in line and \
                    "payload=HELLO" in line and "crc=ok" in line:
                origin_tx = True
            if runtime_ready and origin_tx:
                print("LOCAL_RUNTIME_MARKER_FOUND=yes", flush=True)
                print("LOCAL_ORIGIN_TX_FOUND=yes", flush=True)
                raise SystemExit(0)
...
print(f"LOCAL_ORIGIN_TX_FOUND={'yes' if origin_tx else 'no'}",
      file=sys.stderr, flush=True)
raise SystemExit(1)
PY
then
  fail "Local relay-node runtime marker or origin TX was not received from $PORT."
fi
