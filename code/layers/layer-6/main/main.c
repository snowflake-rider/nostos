/*
 * Layer 6: GATT Server + connectable Advertising + Active Scanning
 *
 * 두 ESP32-S3에 같은 firmware를 넣는다. 각 보드는 Bluetooth MAC으로
 * ESP32-L6-XX 이름을 만들고, GATT Server/Advertising을 제공하면서
 * 20-byte custom packet을 Advertising하고, 처음 받은 logical packet을
 * TTL 감소 후 한 번 forwarding한다. 표준 Bluetooth Mesh는 아니다.
 */
...
#define TAG "LAYER_6"
#define DEVICE_NAME_PREFIX "ESP32-L6-"
...
#define PACKET_TX_QUEUE_LENGTH 16
...
#define PACKET_ORIGIN_INTERVAL_MS 8000U
#define PACKET_ADV_DWELL_MS 500U
#define PACKET_SCHEDULER_INTERVAL_MS 50U
#define PACKET_STATUS_INTERVAL_MS 1000U
...
static volatile uint32_t packet_relayed_path_count;
static volatile uint32_t packet_duplicate_count;
static volatile uint32_t packet_rx_queue_drop_count;
static volatile uint32_t packet_tx_queue_drop_count;
static volatile uint32_t packet_origin_tx_count;
static volatile uint32_t packet_forward_tx_count;
...
static QueueHandle_t packet_tx_queue;
static SemaphoreHandle_t logical_dedup_mutex;
static layer_packet_dedup_t packet_dedup;
static layer_path_dedup_t path_dedup;
static TickType_t last_origin_tick;
static TickType_t last_advertising_start_tick;

typedef enum {
  PACKET_TX_KIND_ORIGIN = 0,
  PACKET_TX_KIND_FORWARD,
} packet_tx_kind_t;

typedef struct {
  layer_packet_t packet;
  packet_tx_kind_t kind;
} packet_tx_record_t;

static packet_tx_record_t active_tx_record;
static bool active_tx_record_valid;
...
static layer_packet_t make_origin_packet(uint16_t sequence) {
...
static esp_err_t prepare_advertising_packet(const layer_packet_t *packet) {
...
static bool logical_duplicate_or_record(uint8_t origin, uint16_t sequence) {
  bool duplicate;

  if (xSemaphoreTake(logical_dedup_mutex, portMAX_DELAY) != pdTRUE) {
    return true;
  }
  duplicate = layer_packet_dedup_is_duplicate_or_record(
      &packet_dedup, origin, sequence);
  xSemaphoreGive(logical_dedup_mutex);
  return duplicate;
}

static uint32_t relay_delay_ms(uint8_t origin, uint16_t sequence) {
  const uint8_t sequence_low = (uint8_t)(sequence & 0xFFU);
  const uint8_t sequence_high = (uint8_t)(sequence >> 8U);
  const uint8_t mixed =
      local_node_id ^ origin ^ sequence_low ^ sequence_high;
  return 80U + ((uint32_t)mixed % 121U);
}

static bool enqueue_tx_record(const layer_packet_t *packet,
                              packet_tx_kind_t kind) {
  const packet_tx_record_t record = {
      .packet = *packet,
      .kind = kind,
  };

  if (xQueueSend(packet_tx_queue, &record, 0) == pdTRUE) {
    return true;
  }
  packet_tx_queue_drop_count++;
  ESP_LOGW(TAG,
           "[LAYER-6] TX_QUEUE_DROP kind=%s origin=%02X seq=%u ttl=%u",
           kind == PACKET_TX_KIND_ORIGIN ? "origin" : "forward",
           packet->sender, (unsigned)packet->sequence, (unsigned)packet->ttl);
  return false;
}

static void advance_packet_sequence(void) {
  packet_sequence++;
...
static void enqueue_periodic_origin(void) {
  advance_packet_sequence();
  const layer_packet_t packet = make_origin_packet(packet_sequence);
  if (enqueue_tx_record(&packet, PACKET_TX_KIND_ORIGIN)) {
    (void)logical_duplicate_or_record(packet.sender, packet.sequence);
  }
  last_origin_tick = xTaskGetTickCount();
}
...
static void packet_worker(void *argument) {
  ...
  while (true) {
    ...
    if (status != LAYER_PACKET_OK) {
      ESP_LOGW(TAG, "[LAYER-6] PACKET_RX_REJECT status=%d rssi=%d",
               (int)status, record.rssi);
      continue;
    }
    if (packet.type != LAYER_PACKET_TYPE_HELLO ||
        (packet.recipient != LAYER_PACKET_BROADCAST &&
         packet.recipient != local_node_id)) {
      continue;
    }

    const uint8_t via = record.address[ESP_BD_ADDR_LEN - 1];
    const layer_relay_path_t path =
        layer_relay_classify_path(packet.sender, via);
    const bool path_duplicate = layer_path_dedup_is_duplicate_or_record(
        &path_dedup, packet.sender, packet.sequence, via);
    const bool logical_duplicate =
        logical_duplicate_or_record(packet.sender, packet.sequence);

    if (!path_duplicate) {
      char payload[LAYER_PACKET_PAYLOAD_CAPACITY + 1U];
      make_printable(packet.payload, packet.payload_length, payload);
      ESP_LOGI(TAG,
               "[LAYER-6] PACKET_RX_%s local=%02X origin=%02X via=%02X"
               " seq=%u ttl=%u recipient=%02X len=%u payload=%s"
               " duplicate=%s crc=ok rssi=%d",
               path == LAYER_RELAY_PATH_DIRECT ? "DIRECT" : "RELAYED",
               local_node_id, packet.sender, via,
               (unsigned)packet.sequence, (unsigned)packet.ttl,
               packet.recipient, (unsigned)packet.payload_length, payload,
               logical_duplicate ? "yes" : "no", record.rssi);
      if (path == LAYER_RELAY_PATH_RELAYED) {
        packet_relayed_path_count++;
      }
    }
    ...
    if (packet.sender == local_node_id || packet.ttl == 0U) {
      continue;
    }

    layer_packet_t forwarded;
    if (layer_relay_prepare_forward(&packet, &forwarded) != LAYER_RELAY_OK) {
      continue;
    }
    const uint32_t delay_ms = relay_delay_ms(packet.sender, packet.sequence);
    vTaskDelay(pdMS_TO_TICKS(delay_ms));
    if (enqueue_tx_record(&forwarded, PACKET_TX_KIND_FORWARD)) {
      ESP_LOGI(TAG,
               "[LAYER-6] PACKET_FORWARD_QUEUED local=%02X origin=%02X"
               " seq=%u ttl=%u delay_ms=%" PRIu32,
               local_node_id, forwarded.sender,
               (unsigned)forwarded.sequence, (unsigned)forwarded.ttl,
               delay_ms);
    }
  }
}
...
static void maybe_log_dual_role_ready(void) {
  if (!dual_role_ready_logged && service_ready && advertising_active &&
    ...
    ESP_LOGI(TAG, "[LAYER-6] GATT_SERVER_READY");
    ESP_LOGI(TAG,
             "[LAYER-6] DUAL_ROLE_READY node=%02X name=%s adv=yes scan=yes",
...
static void try_start_advertising(void) {
  ...
  if (err != ESP_OK) {
    ESP_LOGE(TAG, "[LAYER-6] ADVERTISING_START_REQUEST_FAILED err=%s",
             esp_err_to_name(err));
    return;
  }
...
static void request_next_packet_update(void) {
  if (connected || !advertising_active || packet_update_pending ||
      active_tx_record_valid) {
    return;
  }

  const TickType_t now = xTaskGetTickCount();
  if ((now - last_advertising_start_tick) <
      pdMS_TO_TICKS(PACKET_ADV_DWELL_MS)) {
    return;
  }
  if (xQueueReceive(packet_tx_queue, &active_tx_record, 0) != pdTRUE) {
    return;
  }

  active_tx_record_valid = true;
  packet_update_pending = true;
  const esp_err_t err = esp_ble_gap_stop_advertising();
  if (err != ESP_OK) {
    packet_update_pending = false;
    if (xQueueSendToFront(packet_tx_queue, &active_tx_record, 0) != pdTRUE) {
      packet_tx_queue_drop_count++;
    }
    active_tx_record_valid = false;
    ESP_LOGE(TAG, "[LAYER-6] PACKET_UPDATE_STOP_FAILED err=%s",
             esp_err_to_name(err));
  }
}
...
static void gap_callback(esp_gap_ble_cb_event_t event,
  ...
  case ESP_GAP_BLE_ADV_DATA_RAW_SET_COMPLETE_EVT:
    if (param->adv_data_raw_cmpl.status != ESP_BT_STATUS_SUCCESS) {
      ESP_LOGE(TAG, "[LAYER-6] ADV_DATA_FAILED status=%d",
  ...
  case ESP_GAP_BLE_SCAN_RSP_DATA_RAW_SET_COMPLETE_EVT:
    if (param->scan_rsp_data_raw_cmpl.status != ESP_BT_STATUS_SUCCESS) {
      ESP_LOGE(TAG, "[LAYER-6] SCAN_RSP_DATA_FAILED status=%d",
    ...
    if (advertising_config_pending == 0) {
      ESP_LOGI(TAG, "[LAYER-6] ADV_DATA_READY name=%s", device_name);
    }
    try_start_advertising();
    break;
  ...
  case ESP_GAP_BLE_ADV_START_COMPLETE_EVT:
    ...
    if (param->adv_start_cmpl.status != ESP_BT_STATUS_SUCCESS) {
      ...
      ESP_LOGE(TAG, "[LAYER-6] ADVERTISING_START_FAILED status=%d",
               param->adv_start_cmpl.status);
      return;
    }
    advertising_active = true;
    last_advertising_start_tick = xTaskGetTickCount();
    ESP_LOGI(TAG,
             "[LAYER-6] ADVERTISING_STARTED node=%02X name=%s type=connectable",
             local_node_id, device_name);
    if (active_tx_record_valid) {
      char payload[LAYER_PACKET_PAYLOAD_CAPACITY + 1U];
      make_printable(active_tx_record.packet.payload,
                     active_tx_record.packet.payload_length, payload);
      const bool is_origin =
          active_tx_record.kind == PACKET_TX_KIND_ORIGIN;
      ESP_LOGI(TAG,
               "[LAYER-6] PACKET_TX_%s local=%02X origin=%02X seq=%u"
               " ttl=%u recipient=%02X len=%u payload=%s crc=ok",
               is_origin ? "ORIGIN" : "FORWARD", local_node_id,
               active_tx_record.packet.sender,
               (unsigned)active_tx_record.packet.sequence,
               (unsigned)active_tx_record.packet.ttl,
               active_tx_record.packet.recipient,
               (unsigned)active_tx_record.packet.payload_length, payload);
      if (is_origin) {
        packet_origin_tx_count++;
      } else {
        packet_forward_tx_count++;
      }
      active_tx_record_valid = false;
    }
    packet_update_pending = false;
    if (advertising_restart_pending) {
      ...
      ESP_LOGI(TAG, "[LAYER-6] ADVERTISING_RESTARTED");
    }
  ...
  case ESP_GAP_BLE_ADV_STOP_COMPLETE_EVT:
    advertising_active = false;
    ESP_LOGI(TAG, "[LAYER-6] ADVERTISING_STOPPED status=%d",
             param->adv_stop_cmpl.status);
    if (packet_update_pending &&
        param->adv_stop_cmpl.status == ESP_BT_STATUS_SUCCESS) {
      if (!active_tx_record_valid ||
          prepare_advertising_packet(&active_tx_record.packet) != ESP_OK) {
        packet_update_pending = false;
        active_tx_record_valid = false;
        ESP_LOGE(TAG, "[LAYER-6] PACKET_ENCODE_FAILED");
        break;
      }
      ...
      if (err != ESP_OK) {
        ...
        active_tx_record_valid = false;
        packet_tx_queue_drop_count++;
        ESP_LOGE(TAG, "[LAYER-6] PACKET_CONFIG_FAILED err=%s",
                 esp_err_to_name(err));
      }
    } else if (packet_update_pending) {
      packet_update_pending = false;
      if (active_tx_record_valid &&
          xQueueSendToFront(packet_tx_queue, &active_tx_record, 0) != pdTRUE) {
        packet_tx_queue_drop_count++;
      }
      active_tx_record_valid = false;
      ESP_LOGE(TAG, "[LAYER-6] PACKET_UPDATE_STOP_STATUS_FAILED");
    }
    break;

  case ESP_GAP_BLE_SCAN_PARAM_SET_COMPLETE_EVT: {
    if (param->scan_param_cmpl.status != ESP_BT_STATUS_SUCCESS) {
      ESP_LOGE(TAG, "[LAYER-6] SCAN_PARAMS_FAILED status=%d",
               param->scan_param_cmpl.status);
      return;
    }
    ESP_LOGI(TAG, "[LAYER-6] SCAN_PARAMS_READY");
    const esp_err_t err = esp_ble_gap_start_scanning(0);
    if (err != ESP_OK) {
      ESP_LOGE(TAG, "[LAYER-6] SCAN_START_REQUEST_FAILED err=%s",
               esp_err_to_name(err));
    }
    break;
  }
  ...
  case ESP_GAP_BLE_SCAN_START_COMPLETE_EVT:
    if (param->scan_start_cmpl.status != ESP_BT_STATUS_SUCCESS) {
      ...
      ESP_LOGE(TAG, "[LAYER-6] SCAN_START_FAILED status=%d",
               param->scan_start_cmpl.status);
      return;
    }
    scanner_started = true;
    ESP_LOGI(TAG, "[LAYER-6] SCANNING_STARTED mode=active");
    maybe_log_dual_role_ready();
    break;
  ...
  case ESP_GAP_BLE_SCAN_STOP_COMPLETE_EVT:
    scanner_started = false;
    ESP_LOGW(TAG, "[LAYER-6] SCANNING_STOPPED status=%d",
...
static void send_rx_write_response(esp_gatt_if_t gatts_if,
  ...
  if (err != ESP_OK) {
    ESP_LOGE(TAG, "[LAYER-6] WRITE_RESPONSE_FAILED err=%s",
             esp_err_to_name(err));
  }
}
...
static void handle_rx_write(esp_gatt_if_t gatts_if,
  ...
  if (set_value_err != ESP_OK) {
    ESP_LOGE(TAG, "[LAYER-6] TX_VALUE_UPDATE_FAILED err=%s",
             esp_err_to_name(set_value_err));
    send_rx_write_response(gatts_if, param, ESP_GATT_ERROR);
    return;
  }

  ESP_LOGI(TAG, "[LAYER-6] RX_WRITE len=%u value=%s",
  ...
  if (!connected || !notification_enabled) {
    ESP_LOGI(TAG,
             "[LAYER-6] TX_UPDATED_NOTIFY_DISABLED len=%u value=%s",
  ...
  if (notify_err != ESP_OK) {
    ESP_LOGE(TAG, "[LAYER-6] TX_NOTIFY_FAILED err=%s",
             esp_err_to_name(notify_err));
    return;
  }
  ESP_LOGI(TAG, "[LAYER-6] TX_NOTIFY len=%u value=%s",
           (unsigned)tx_value_length, printable_tx);
}

static void handle_cccd_write(const esp_ble_gatts_cb_param_t *param) {
  if (param->write.is_prep || param->write.len != 2) {
    ESP_LOGW(TAG, "[LAYER-6] CCCD_WRITE_IGNORED len=%u",
             (unsigned)param->write.len);
    return;
  }
  ...
  ESP_LOGI(TAG, "[LAYER-6] NOTIFY_%s",
...
static void gatts_callback(esp_gatts_cb_event_t event,
  ...
  case ESP_GATTS_REG_EVT:
    if (param->reg.status != ESP_GATT_OK) {
      ESP_LOGE(TAG, "[LAYER-6] GATTS_REGISTER_FAILED status=%d",
               param->reg.status);
      return;
    }
  ...
  case ESP_GATTS_CREAT_ATTR_TAB_EVT:
    if (param->add_attr_tab.status != ESP_GATT_OK ||
      ...
      ESP_LOGE(TAG,
               "[LAYER-6] GATT_TABLE_FAILED status=%d handles=%u expected=%u",
  ...
  case ESP_GATTS_START_EVT:
    if (param->start.status != ESP_GATT_OK) {
      ESP_LOGE(TAG, "[LAYER-6] GATT_SERVICE_START_FAILED status=%d",
               param->start.status);
      return;
    }
    service_ready = true;
    ESP_LOGI(TAG, "[LAYER-6] GATT_SERVICE_READY");
    try_start_advertising();
    maybe_log_dual_role_ready();
    break;
  ...
  case ESP_GATTS_CONNECT_EVT:
    ...
    ESP_LOGI(TAG,
             "[LAYER-6] CONNECTED conn_id=%u peer=%02x:%02x:%02x:%02x:%02x:%02x scan=%s",
  ...
  case ESP_GATTS_DISCONNECT_EVT:
    ...
    advertising_active = false;
    advertising_start_requested = false;
    ESP_LOGI(TAG, "[LAYER-6] DISCONNECTED reason=0x%02x scan=%s",
  ...
  case ESP_GATTS_READ_EVT:
    if (param->read.handle == attribute_handles[IDX_TX_CHAR_VALUE]) {
      char printable_tx[PAYLOAD_MAX_LENGTH + 1];
      make_printable(tx_value, tx_value_length, printable_tx);
      ESP_LOGI(TAG, "[LAYER-6] TX_READ len=%u value=%s",
               (unsigned)tx_value_length, printable_tx);
    }
    break;

  case ESP_GATTS_MTU_EVT:
    ESP_LOGI(TAG, "[LAYER-6] MTU_UPDATED mtu=%u", (unsigned)param->mtu.mtu);
    break;

  case ESP_GATTS_CONF_EVT:
    ESP_LOGI(TAG, "[LAYER-6] INDICATION_CONFIRMED status=%d",
             param->conf.status);
    break;
...
void app_main(void) {
  ...
  ESP_LOGI(TAG, "[LAYER-6] BOOT_SUCCESS target=%s idf=%s",
           CONFIG_IDF_TARGET, esp_get_idf_version());
  ESP_ERROR_CHECK(nvs_flash_init());
  ESP_LOGI(TAG, "[LAYER-6] NVS_READY");
  ...
  ESP_LOGI(TAG, "[LAYER-6] BLE_CONTROLLER_ENABLED");

  ESP_ERROR_CHECK(esp_bluedroid_init());
  ESP_ERROR_CHECK(esp_bluedroid_enable());
  ESP_LOGI(TAG, "[LAYER-6] BLUEDROID_ENABLED");
  ...
  if (written != DEVICE_NAME_LENGTH) {
    ESP_LOGE(TAG, "[LAYER-6] DEVICE_NAME_FAILED length=%d", written);
    return;
  }
  ESP_LOGI(TAG,
           "[LAYER-6] NODE_READY id=%02X name=%s mac=%02x:%02x:%02x:%02x:%02x:%02x",
  ...
  layer_path_dedup_init(&path_dedup);
  logical_dedup_mutex = xSemaphoreCreateMutex();
  packet_rx_queue = xQueueCreate(PACKET_RX_QUEUE_LENGTH,
                                 sizeof(packet_rx_record_t));
  packet_tx_queue = xQueueCreate(PACKET_TX_QUEUE_LENGTH,
                                 sizeof(packet_tx_record_t));
  if (logical_dedup_mutex == NULL || packet_rx_queue == NULL ||
      packet_tx_queue == NULL) {
    ESP_LOGE(TAG, "[LAYER-6] PACKET_PRIMITIVE_CREATE_FAILED");
    return;
  }

  active_tx_record.packet = make_origin_packet(packet_sequence);
  active_tx_record.kind = PACKET_TX_KIND_ORIGIN;
  active_tx_record_valid = true;
  (void)logical_duplicate_or_record(active_tx_record.packet.sender,
                                    active_tx_record.packet.sequence);
  last_origin_tick = xTaskGetTickCount();
  const uint32_t initial_delay_ms =
      250U + (((uint32_t)local_node_id % 11U) * 100U);
  ESP_LOGI(TAG, "[LAYER-6] ORIGIN_STAGGER delay_ms=%" PRIu32,
           initial_delay_ms);
  vTaskDelay(pdMS_TO_TICKS(initial_delay_ms));
  ESP_ERROR_CHECK(prepare_advertising_packet(&active_tx_record.packet));
  prepare_scan_response();

  if (xTaskCreate(packet_worker, "layer6_packet_rx", 4096, NULL, 5, NULL) !=
      pdPASS) {
    ESP_LOGE(TAG, "[LAYER-6] PACKET_WORKER_CREATE_FAILED");
    return;
  }
  ...
  if (!service_ready || !advertising_active || !scanner_started) {
    ESP_LOGE(TAG,
             "[LAYER-6] DUAL_ROLE_START_TIMEOUT gatt=%s advertising=%s scanning=%s",
  ...
  while (true) {
    const TickType_t now = xTaskGetTickCount();
    if ((now - last_origin_tick) >=
        pdMS_TO_TICKS(PACKET_ORIGIN_INTERVAL_MS)) {
      enqueue_periodic_origin();
    }
    request_next_packet_update();

    if ((now - last_status_tick) >=
        pdMS_TO_TICKS(PACKET_STATUS_INTERVAL_MS)) {
      last_status_tick = now;
      heartbeat_count++;
      ESP_LOGI(TAG,
               "[LAYER-6] RELAY_NODE_ACTIVE heartbeat=%" PRIu32
               " node=%02X gatt=%s advertising=%s scanning=%s connected=%s"
               " seq=%u origin_tx=%" PRIu32 " forward_tx=%" PRIu32
               " rx_new=%" PRIu32 " rx_relayed_paths=%" PRIu32
               " logical_duplicates=%" PRIu32
               " rx_drops=%" PRIu32 " tx_drops=%" PRIu32
               " tx_queued=%u last_origin=%02X last_rssi=%d",
               heartbeat_count, local_node_id, service_ready ? "yes" : "no",
               advertising_active ? "yes" : "no",
               scanner_started ? "yes" : "no", connected ? "yes" : "no",
               (unsigned)packet_sequence, (uint32_t)packet_origin_tx_count,
               (uint32_t)packet_forward_tx_count,
               (uint32_t)packet_receive_count,
               (uint32_t)packet_relayed_path_count,
               (uint32_t)packet_duplicate_count,
               (uint32_t)packet_rx_queue_drop_count,
               (uint32_t)packet_tx_queue_drop_count,
               (unsigned)uxQueueMessagesWaiting(packet_tx_queue),
               (uint8_t)last_packet_sender, (int)last_packet_rssi);
    }
    vTaskDelay(pdMS_TO_TICKS(PACKET_SCHEDULER_INTERVAL_MS));
  }
}
