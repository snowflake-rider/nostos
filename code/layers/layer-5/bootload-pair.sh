...
LOG_FILE="$LOG_DIR/esp32s3-layer-5-packet-node-pair-$RUN_TIMESTAMP.log"
...
finish() {
  ...
  if [[ "$RESULT" == "PASS" && $exit_code -eq 0 ]]; then
    ...
    printf 'PAIR_PACKET_TX=PASS\n'
    printf 'PAIR_PACKET_RX=PASS\n'
...
printf 'ESP32-S3 Layer 5 two-board packet-node workflow\n'
...
printf 'Waiting up to %s seconds for both boards to report local readiness and mutual PACKET_RX_NEW...\n' \
...
node_pattern = re.compile(r"PACKET_NODE_ACTIVE .* node=([0-9A-F]{2}) ")
tx_pattern = re.compile(r"PACKET_TX sender=([0-9A-F]{2}) seq=([0-9]+) .*payload=HELLO crc=ok")
peer_pattern = re.compile(r"PACKET_RX_NEW local=([0-9A-F]{2}) sender=([0-9A-F]{2}) seq=([0-9]+) .*payload=HELLO crc=ok")
state = {
    "A": {"ready": False, "tx": False, "node": None, "peer": None},
    "B": {"ready": False, "tx": False, "node": None, "peer": None},
}
...
def record(label, line):
    ...
    if tx_match:
        state[label]["tx"] = True
        state[label]["node"] = state[label]["node"] or tx_match.group(1)
...
def complete():
    a, b = state["A"], state["B"]
    return (
        a["ready"] and b["ready"] and a["tx"] and b["tx"] and
...
try:
    ...
    while time.monotonic() < deadline:
        ...
        if complete():
            ...
            print("BIDIRECTIONAL_PACKET_RX_NEW=yes", flush=True)
            raise SystemExit(0)
...
for label in ("A", "B"):
    item = state[label]
    print(
        f"BOARD_{label}_STATE ready={item['ready']} tx={item['tx']} node={item['node']} peer={item['peer']}",
        file=sys.stderr,
        flush=True,
    )
print("BIDIRECTIONAL_PACKET_RX_NEW=no", file=sys.stderr, flush=True)
raise SystemExit(1)
PY
then
  fail 'Both boards did not prove GATT + Advertising + Scanning + mutual PACKET_RX_NEW.'
fi
